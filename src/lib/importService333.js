...
class ImportService {
  ...
  async getImportConfig(importConfigId) {
    ...
    try {
      ...
      const fullConfig = {
        ...
        password: connectionResult.rows[0]?.password, // Don't return password for security when editing
  ...
  async insertOrUpdateVehicleRecord(client, record, config) {
    ...
    const referenceDealerId =
      record.dealerid || record.dealer_id || record.reference_dealer_id || null;
    ...
    queryParams.forEach((param, i) => {
      ...
      console.log(`  ${paramNames[i]}: ${JSON.stringify(param)} (${type})`);
    });
    ...
    try {
      ...
      return {
        action: existingCheck.rows.length > 0 ? 'updated' : 'inserted',
        vehicleId
      };
    } catch (error) {
  ...
  async executeImport(importConfigId, options = {}) {
    const { selectedRows = [], fieldMappings = [], transformedData = null } = options;
    ...
    try {
      ...
      try {
        ...
        if (transformedData && transformedData.length > 0) {
          ...
          // Download file
          const downloadResult = await this.downloadFile(config);
    ...
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          let transformedRecord = this.transformRecordWithValidation(record, fieldMappings);
