...
const CSVAnalysisModal: React.FC<CSVAnalysisModalProps> = ({
  ...
  const handleLoadAllRows = async () => {
    ...
    console.log('Frontend: Request body:', requestBody);
      ...
      const response = await fetch(buildApiUrl('import/load-all-rows'), {
        method: 'POST',
        headers: {
  ...
  const handleTransformData = async () => {
    ...
    try {
      ...
      const response = await fetch(buildApiUrl('import/transform-data'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('auth_token') || 'public'}`
      ...
      if (data.success) {
        ...
        // Auto-select all rows after transformation
        const allRowIndices = data.data.map((_, index) => index);
        setSelectedRows(allRowIndices);
        toast({
          title: "Data Transformed Successfully",
          description: `Successfully transformed ${data.data.length} rows. Applied: ${data.transformationsApplied ? data.transformationsApplied.join(', ') : 'various transformations'}. All rows have been automatically selected.`,
          variant: "default"
        });
      } else {
  ...
  // Auto-trigger transform when modal opens with autoTransform flag
  React.useEffect(() => {
    if (isOpen && autoTransform && csvData && !isTransformed && !isTransforming) {
      // Small delay to ensure modal is fully rendered
      const timer = setTimeout(() => {
        // Call transform data directly
        if (!csvData) return;
  ...
  }, [isOpen, autoTransform, csvData, isTransformed, isTransforming, toast]);
        ...
        <div className="space-y-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="p-4 border rounded-lg bg-primary/10">
                  <div className="flex flex-wrap gap-2" style={{ width: '100%', overflowY: 'auto' }}>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="border rounded-lg overflow-auto" style={{ height: '500px', width: '100%' }}>
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      {csvData.headers.map((header, index) => (
                        <TableHead key={index} className="text-xs sticky top-0 bg-white z-10">
                          {header}
                        </TableHead>
                      ))}
                    </TableRow>
