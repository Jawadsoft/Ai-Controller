...
type RemoteFilePickState = {
  mode: 'preview' | 'download' | 'execute';
  files: string[];
  selectedFile: string;
...
const ImportConfiguration: React.FC<ImportConfigurationProps> = ({ onEditConfig, onCancel, preloadedConfig }) => {
  ...
  const TabNavigation = ({ className = "" }) => (
    <div
      className={`flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between ${className}`}
    >
      <div className="order-2 flex w-full gap-2 sm:order-1 sm:w-auto">
        <Button
          ...
          className="flex flex-1 items-center justify-center gap-1 text-xs sm:flex-initial sm:gap-2 sm:text-sm"
        >
          <span>←</span>
          <span className="hidden sm:inline">Previous</span>
          <span className="sm:hidden">Prev</span>
          {currentTabIndex > 0 && (
            <span className="hidden text-muted-foreground md:inline lg:text-sm">
      ...
      <div className="order-1 flex items-center justify-center gap-2 text-xs text-muted-foreground sm:order-2 sm:text-sm">
        <span>
          {currentTabIndex + 1} / {tabOrder.length}
        </span>
        <span className="hidden sm:inline">•</span>
        <span className="max-w-[12rem] truncate sm:max-w-none">
          {tabLabels[tabOrder[currentTabIndex]]}
        </span>
      </div>
    </div>
  );
  ...
  const saveConfig = async () => {
    ...
    try {
      ...
      const isUpdate = currentConfig.id !== undefined;
              const url = isUpdate ? buildApiUrl(`import/configs/${currentConfig.id}`) : buildApiUrl('import/configs');
  ...
  const retrieveCSVDataAfterConnection = async (connectionData: any, selectedFileName?: string) => {
    try {
      setCsvAnalysisLoading(true);
      setCsvAnalysisError(null);
      
      console.log('Retrieving CSV data after successful connection:', connectionData, selectedFileName);
      ...
      const response = await fetch(buildApiUrl('import/preview-csv'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
      ...
      if (result.success && result.needsFileSelection && result.availableFiles?.length) {
        setRemoteFilePick({
          mode: 'preview',
          files: result.availableFiles,
          selectedFile: result.availableFiles[0],
          connectionData: { ...connectionData }
        });
        toast({
          title: "Multiple files found",
          description: "Select which CSV file to preview.",
        });
        return;
      }
  ...
  const testConnectionAndDownload = async (opts?: {
    ...
    try {
      ...
      const response = await fetch(buildApiUrl('import/test-connection-and-download'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
      ...
      if (result.success && result.needsFileSelection && result.matchingFiles?.length) {
        const names = result.matchingFiles.map((f: { name: string }) => f.name);
        setRemoteFilePick({
          mode: 'download',
          files: names,
        ...
        toast({
          title: "Multiple files found",
          description: "Select which file to download for preview.",
        });
        return;
      }
  ...
  const previewLocalCSV = async () => {
    ...
    try {
      ...
      console.log('Request body:', requestBody);
              ...
              const response = await fetch(buildApiUrl('import/preview-local-csv'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
  ...
  const previewCSV = async (selectedFileName?: string) => {
    ...
    console.log('Sending preview CSV request:', connectionData, selectedFileName);
    await retrieveCSVDataAfterConnection(connectionData, selectedFileName);
  };

  const executeImportWithBody = async (
    configId: number,
    body: Record<string, unknown> = {},
    options?: { suppressSuccessToast?: boolean }
  ) => {
    const response = await fetch(buildApiUrl(`/import/configs/${configId}/execute`), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
    ...
    if (result.needsFileSelection && result.matchingFiles?.length) {
      const { remoteFileName: _r, ...restPayload } = body;
      setRemoteFilePick({
        mode: 'execute',
        files: result.matchingFiles,
        selectedFile: result.matchingFiles[0],
        configId,
        executePayload: restPayload
      });
      toast({
        title: "Multiple files found",
        description: "Select which remote file to import.",
      });
      return { status: 'needsSelection' as const };
    }
  ...
  const confirmRemoteFilePick = async () => {
    if (!remoteFilePick) return;
    const pick = remoteFilePick;
    setRemoteFilePick(null);

    if (pick.mode === 'preview' && pick.connectionData) {
      await retrieveCSVDataAfterConnection(pick.connectionData, pick.selectedFile);
      return;
    }
    ...
    if (pick.mode === 'execute' && pick.configId != null) {
      setIsExecutingImport(true);
      try {
        const out = await executeImportWithBody(pick.configId, {
          ...(pick.executePayload || {}),
          remoteFileName: pick.selectedFile
        });
        if (out.status === 'ok') {
          loadConfigs();
          if (csvData?.configId === pick.configId) {
            setShowCSVModal(false);
            setCsvData(null);
          }
        }
  ...
  const executeImport = async (configId: number) => {
    ...
    setIsExecutingImport(true);
    try {
      await executeImportWithBody(configId, {});
    } catch (error) {
  ...
  const editConfig = async (config: ImportConfigResponse) => {
    try {
      ...
      const response = await fetch(buildApiUrl(`/import/configs/${config.id}`), {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        }
      });
  ...
  const smartImport = async () => {
    ...
    try {
      ...
      console.log('Starting smart import with:', { configName, connectionData });
              ...
              const response = await fetch(buildApiUrl('import/smart-import'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
      ...
      if (!response.ok) {
        if (response.status === 409) {
          ...
          if (shouldUseExisting) {
            ...
            const existingConfig = await fetch(buildApiUrl(`/import/configs/${existingConfigId}`), {
              headers: {
                'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
              }
            });
  ...
  const quickImport = async () => {
    ...
    try {
      ...
      // First, check if a configuration with this name already exists
              const checkResponse = await fetch(buildApiUrl('import/configs/check-name'), {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        }
      });
      ...
      if (checkResponse.ok) {
        ...
        if (checkResult.exists) {
          ...
          if (shouldUseExisting) {
            ...
            const existingConfig = await fetch(buildApiUrl(`/import/configs/${checkResult.configId}`), {
              headers: {
                'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
              }
            });
            ...
            if (existingConfig.ok) {
              ...
              const executeOutcome = await executeImportWithBody(checkResult.configId, {
                selectedRows: [],
                fieldMappings: existingConfigData.data.fieldMappings || []
              });

              if (executeOutcome.status === 'ok') {
                const d = executeOutcome.data;
                toast({
                  title: "Quick Import Successful!",
                  description: `Imported ${d?.recordsProcessed ?? 0} records using existing configuration.`
                });
                loadConfigs();
              }
            } else {
          ...
          // No existing configuration, create a new one and execute
                      const createResponse = await fetch(buildApiUrl('import/smart-import'), {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
          ...
          if (createResponse.ok && createResult.success) {
            const executeOutcome = await executeImportWithBody(createResult.data.configId, {
              selectedRows: [],
            ...
            if (executeOutcome.status === 'ok') {
              ...
              toast({
                title: "Quick Import Successful!",
                description: `Created new configuration and imported ${d?.recordsProcessed ?? 0} records.`
              });
              loadConfigs();
            }
          } else {
  ...
  const handleCSVImport = async (selectedRows: number[], transformedData?: any[]) => {
    try {
      ...
      if (csvData?.configId) {
        ...
        const outcome = await executeImportWithBody(
          csvData.configId,
          {
            selectedRows,
        ...
        if (outcome.status === 'needsSelection') {
          return;
        }
      ...
      const executeResponse = await fetch(buildApiUrl('import/preview-execute'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
  ...
  return (
    <div className="container mx-auto space-y-4 px-4 py-4 sm:space-y-6 sm:p-6">
      ...
      {isEditing ? (
        <Card>
          <CardHeader className="space-y-2 p-4 sm:p-6">
            <CardTitle className="text-lg font-semibold sm:text-xl">Import Configuration</CardTitle>
          ...
          <CardContent className="space-y-4 p-4 pt-0 sm:p-6 sm:pt-0">
            <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
              <TabsList className="flex h-auto w-full flex-nowrap gap-1 overflow-x-auto p-1 sm:grid sm:grid-cols-6 sm:overflow-x-visible">
                <TabsTrigger value="connection" className="shrink-0 px-2.5 text-xs sm:px-3 sm:text-sm">
                  Connection
                </TabsTrigger>
                <TabsTrigger value="test" className="shrink-0 px-2.5 text-xs sm:px-3 sm:text-sm">
                  Test
                </TabsTrigger>
                <TabsTrigger value="mappings" className="shrink-0 px-2.5 text-xs sm:px-3 sm:text-sm">
                  Mappings
                </TabsTrigger>
                <TabsTrigger value="processing" className="shrink-0 px-2.5 text-xs sm:px-3 sm:text-sm">
                  Processing
                </TabsTrigger>
                <TabsTrigger value="schedule" className="shrink-0 px-2.5 text-xs sm:px-3 sm:text-sm">
                  Schedule
                </TabsTrigger>
                <TabsTrigger value="file" className="shrink-0 px-2.5 text-xs sm:px-3 sm:text-sm">
                  File
                </TabsTrigger>
              </TabsList>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value: 'manual' | 'test' | 'hourly' | 'daily' | 'weekly' | 'monthly') => setCurrentConfig({
                        ...currentConfig,
                  ...
                  {(currentConfig.fieldMappings || []).length > 0 && (
                    <div className="p-4 border rounded-lg bg-primary/10">
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="font-medium text-primary">Field Mapping Summary</h3>
              ...
              <TabsContent value="test" className="space-y-4">
                ...
                <div className="p-4 bg-primary/10 border border-primary/20 rounded-lg">
                  <h4 className="font-medium text-primary mb-2">💡 Quick Start Guide</h4>
                  ...
                  <div className="mt-4 p-4 border rounded-lg bg-primary/10 border-primary/20">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-primary">Downloading File</span>
                      </div>
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:justify-end">
              ...
              <Button onClick={smartImport} disabled={isLoading} variant="secondary" className="w-full justify-center sm:w-auto">
                {isLoading ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
              ...
              <Button onClick={quickImport} disabled={isLoading} variant="secondary" className="w-full justify-center sm:w-auto">
                {isLoading ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
              ...
              <Button onClick={saveConfig} disabled={isLoading} className="w-full justify-center sm:w-auto">
                {isLoading ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
                  <Settings className="w-4 h-4" />
            ...
            <Card key={config.id}>
              <CardHeader className="space-y-2 p-4 sm:p-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    <CardTitle className="text-lg font-semibold sm:text-xl">{config.config_name}</CardTitle>
          ...
          {!preloadedConfig && configs.length === 0 && !isLoading && (
            <Card>
              <CardContent className="flex flex-col items-center justify-center px-4 py-8 sm:px-6">
                ...
                <Button onClick={handleNewConfiguration} className="w-full sm:w-auto">
      ...
      <Dialog open={!!remoteFilePick} onOpenChange={(open) => { if (!open) setRemoteFilePick(null); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Select remote file</DialogTitle>
            <DialogDescription>
              {remoteFilePick?.mode === 'preview' && 'Multiple CSV files match your settings. Choose one to preview.'}
              {remoteFilePick?.mode === 'download' && 'Multiple files match your pattern. Choose one to download for local preview.'}
              {remoteFilePick?.mode === 'execute' && 'Multiple files match this import configuration. Choose which file to import.'}
            </DialogDescription>
          </DialogHeader>
          {remoteFilePick && (
            <RadioGroup
              value={remoteFilePick.selectedFile}
              onValueChange={(v) => setRemoteFilePick({ ...remoteFilePick, selectedFile: v })}
              className="gap-3"
            >
              {remoteFilePick.files.map((name, idx) => (
                <div key={name} className="flex items-center space-x-2">
                  <RadioGroupItem value={name} id={`remote-file-${idx}`} />
                  <Label htmlFor={`remote-file-${idx}`} className="font-normal cursor-pointer">
                    {name}
                  </Label>
                </div>
              ))}
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            <Button type="button" variant="outline" onClick={() => setRemoteFilePick(null)}>
              Cancel
            </Button>
            <Button
              type="button"
              onClick={() => void confirmRemoteFilePick()}
              disabled={!remoteFilePick?.selectedFile}
            >
              Continue
            </Button>
          </DialogFooter>
