...
import { Building2, Phone, Mail, MapPin, Calendar, Save, Upload, Camera, Clock } from "lucide-react";
...
export const DealerProfileForm = ({ dealer, onSave, showHeader = true }: DealerProfileFormProps) => {
  ...
  const form = useForm<DealerFormData>({
    ...
    defaultValues: {
      ...
      opening_hours: dealer?.opening_hours || DEFAULT_OPENING_HOURS,
    },
  });
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {DAYS_OF_WEEK.map(({ key, label }) => {
                    const dayPath = `opening_hours.${key}` as const;
                    const isClosed = form.watch(`opening_hours.${key}.closed` as any);
                    return (
                        ...
                        <div className="flex items-center gap-3">
                          <label className="flex items-center gap-1.5 cursor-pointer select-none">
                            <input
                              ...
                              checked={!!isClosed}
                              onChange={(e) => {
                                form.setValue(`opening_hours.${key}.closed` as any, e.target.checked);
                                if (e.target.checked) {
                                  form.setValue(`opening_hours.${key}.open` as any, null);
                                  form.setValue(`opening_hours.${key}.close` as any, null);
                                } else {
                                  form.setValue(`opening_hours.${key}.open` as any, "09:00");
                                  form.setValue(`opening_hours.${key}.close` as any, "18:00");
                                }
                              }}
                            />
                          ...
                          {!isClosed && (
                            <>
                              <input
                                ...
                                value={form.watch(`opening_hours.${key}.open` as any) || "09:00"}
                                onChange={(e) => form.setValue(`opening_hours.${key}.open` as any, e.target.value)}
                              />
                              <span className="text-muted-foreground text-sm">to</span>
                              <input
                                type="time"
                                className="border border-input rounded-md px-2 py-1 text-sm bg-background"
                                value={form.watch(`opening_hours.${key}.close` as any) || "18:00"}
                                onChange={(e) => form.setValue(`opening_hours.${key}.close` as any, e.target.value)}
                              />
                            </>
                          )}
                          {isClosed && (
                            <span className="text-sm text-muted-foreground italic">Closed all day</span>
                          )}
                        </div>
                      </div>
                    );
