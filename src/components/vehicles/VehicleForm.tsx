...
import { CarfaxUpload } from "./CarfaxUpload";

// Helper functions to convert between database values (N/U) and form values (new/used)
const mapNewUsedFromDB = (dbValue: string): string => {
  if (dbValue === 'N') return 'new';
  if (dbValue === 'U') return 'used';
  if (dbValue === 'new') return 'new';
  if (dbValue === 'used') return 'used';
  return 'used'; // default fallback
};

const mapNewUsedToDB = (formValue: string): string => {
  if (formValue === 'new') return 'N';
  if (formValue === 'used') return 'U';
...
export const VehicleForm = ({ vehicle, onSuccess, onCancel }: VehicleFormProps) => {
  ...
  const form = useForm<VehicleFormData>({
    ...
    defaultValues: {
      ...
      new_used: mapNewUsedFromDB(vehicle?.new_used || "U"),
  ...
  const onSubmit = async (data: VehicleFormData) => {
    ...
    try {
      ...
      const vehicleData = {
        ...
        year: data.year,
        status: data.status,
        new_used: mapNewUsedToDB(data.new_used),
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-primary">Important Information</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField
                  control={form.control}
                  name="stock_number"
            ...
            {vehicle?.id && (
              <CarfaxUpload
                ...
                onUploadComplete={() => {
                  ...
                  toast({
                    title: "CARFAX Report Updated",
                    description: "Vehicle history has been updated with new CARFAX data",
                  });
                }}
              />
            )}
