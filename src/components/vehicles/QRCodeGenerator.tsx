...
import { buildBackendAssetUrl, downloadBackendAsset } from "@/lib/config";
...
export const QRCodeGenerator = ({ vehicleId, qrCodeUrl, onQRCodeGenerated }: QRCodeGeneratorProps) => {
  ...
  const downloadQRCode = async () => {
    ...
    try {
      await downloadBackendAsset(qrCodeUrl, `vehicle-qr-${vehicleId || 'code'}.png`);
      toast({
        ...
        {qrCodeUrl ? (
          <div className="space-y-3">
            <div className="relative">
              <img 
              crossOrigin = 'anonymous'   // set BEFORE src

                src={buildBackendAssetUrl(qrCodeUrl)} 
                alt="Vehicle QR Code" 
