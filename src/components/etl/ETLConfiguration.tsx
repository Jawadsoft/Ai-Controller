...
const ETLConfiguration: React.FC = () => {
    ...
    try {
      ...
      if (!authToken) {
        toast({
          ...
          description: "Please log in to access ETL configurations",
          variant: "destructive"
        });
        return;
      }
      ...
      const apiUrl = buildApiUrl('etl/configs');
      ...
      if (!contentType || !contentType.includes('application/json')) {
        ...
        toast({
          title: "Error",
          description: `Server returned an error. Status: ${response.status}`,
          variant: "destructive"
        });
        return;
      }
      ...
      if (response.ok) {
        ...
        toast({
          title: "Error",
          description: errorData.error || `Failed to load ETL configurations (${response.status})`,
          variant: "destructive"
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to load ETL configurations",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
  ...
  const loadAvailableFields = async () => {
    try {
      const authToken = localStorage.getItem('auth_token');
      if (!authToken) {
        console.warn('No auth token available for field mappings');
        return;
      }
      
      // Use backend URL directly
      const apiUrl = buildApiUrl('etl/field-mappings');
      const response = await fetch(apiUrl, {
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json'
      ...
      console.log('Field mappings response status:', response.status);
      console.log('Field mappings content-type:', response.headers.get('content-type'));
      
      // Check if response is JSON
      const contentType = response.headers.get('content-type');
      if (!contentType || !contentType.includes('application/json')) {
        const errorText = await response.text();
        console.error('Non-JSON response for field mappings:', errorText.substring(0, 200));
        // Don't show toast for field mappings as it's not critical
        return;
      }
      ...
      if (response.ok) {
        ...
        const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
        console.error('Error loading field mappings:', errorData);
      }
    } catch (error) {
  ...
  const saveConfig = async () => {
    ...
    try {
      ...
      const endpoint = isUpdate ? `etl/configs/${currentConfig.id}` : 'etl/configs';
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      const connectionData = {
        ...
        // Include configId if editing existing config (so backend can fetch stored password if needed)
      ...
      // Use backend URL directly
      const apiUrl = buildApiUrl('etl/test-connection');
      const response = await fetch(apiUrl, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.ok && result.success) {
        ...
        if (result.missing?.password) {
          if (currentConfig.id) {
            errorMessage = "Password is required. Please enter the password to test the connection.";
          } else {
            errorMessage = "Password is required for connection testing.";
          }
  ...
  const deleteConfig = async (configId: number) => {
    ...
    try {
      setIsLoading(true);
      // Use backend URL directly
      const apiUrl = buildApiUrl(`etl/configs/${configId}`);
      const response = await fetch(apiUrl, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
                ...
                <div className="flex gap-2 pt-2">
                    ...
                    onClick={() => {
                      ...
                      setCurrentConfig({
                        ...
                        schedule: {
                          ...
                          frequency: 'daily',
                          timeHour: testTime.getHours(),
                          timeMinute: testTime.getMinutes()
                        }
                      });
                      toast({
                        title: "Quick Test Schedule Set",
                        description: `Schedule set to run at ${testTime.getHours().toString().padStart(2, '0')}:${testTime.getMinutes().toString().padStart(2, '0')} (1 minute from now)`
                      });
                    }}
                  >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      const now = new Date();
                      const testTime = new Date(now.getTime() + 120000); // 2 minutes from now
                      setCurrentConfig({
                        ...currentConfig,
                        schedule: {
                          ...currentConfig.schedule,
                      ...
                      toast({
                        title: "Quick Test Schedule Set",
                        description: `Schedule set to run at ${testTime.getHours().toString().padStart(2, '0')}:${testTime.getMinutes().toString().padStart(2, '0')} (2 minutes from now)`
                      });
                    }}
                  >
                    <Clock className="w-4 h-4 mr-2" />
