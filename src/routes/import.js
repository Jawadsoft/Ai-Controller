...
/** Align with test-connection-and-download file_pattern matching */
function matchesRemoteImportPattern(filename, pattern) {
  const p = pattern || '*';
  if (p === '*') return true;
  if (p.startsWith('*.')) {
    const extension = p.substring(2);
    return filename.toLowerCase().endsWith('.' + extension.toLowerCase());
  }
  return filename.includes(p);
}

// Enhanced CORS middleware specifically for import routes
router.use((req, res, next) => {
  const origin = req.headers.origin;
  
  // Define allowed origins for import operations
  ...
  console.log('🌐 Import Route CORS - Origin:', origin);
  
  // Set CORS headers for import routes
  if (origin && allowedOrigins.includes(origin)) {
    res.header('Access-Control-Allow-Origin', origin);
    console.log('✅ Import CORS - Origin allowed:', origin);
  } else if (!origin) {
    // Allow requests with no origin (same-origin, Postman, curl, etc.)
    res.header('Access-Control-Allow-Origin', '*');
    console.log('✅ Import CORS - No origin, allowing all');
  } else if (process.env.NODE_ENV === 'development') {
    // In development, be more permissive for localhost variations
    if (origin.includes('localhost') || origin.includes('127.0.0.1') || origin.includes('0.0.0.0')) {
      res.header('Access-Control-Allow-Origin', origin);
      console.log('🔧 Import CORS - Development localhost allowed:', origin);
    } else {
      res.header('Access-Control-Allow-Origin', '*');
      console.log('🔧 Import CORS - Development wildcard for:', origin);
    }
  } else {
    console.log('❌ Import CORS - Origin blocked:', origin);
    // In production, still allow for debugging purposes but log it
    res.header('Access-Control-Allow-Origin', origin);
  }
  
  // Set comprehensive CORS headers for import operations
  ...
  if (req.method === 'OPTIONS') {
    console.log('🔄 Import Preflight OPTIONS for:', req.path);
...
router.post('/configs/:id/execute', authenticateToken, async (req, res) => {
  try {
    ...
    const { selectedRows, fieldMappings, transformedData, remoteFileName } = req.body || {};
...
// GET /api/import/errors/:historyId - Get import errors for a specific import history
router.get('/errors/:historyId', authenticateToken, async (req, res) => {
  try {
    const dealerId = req.user.dealer_id;
    ...
    const errors = await importService.getImportErrors(historyId, dealerId);
...
router.post('/preview-csv', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('Extracted values:', { connectionType, hostUrl, port, username, password: password ? '[HIDDEN]' : 'undefined', remoteDirectory, filePattern, selectedFileName });
          ...
          if (testConfig.connection.type === 'sftp') {
            conn.sftp((err, sftp) => {
              ...
              sftp.readdir(testConfig.connection.remote_directory, (err, list) => {
                ...
                const pat = testConfig.connection.file_pattern || '*.csv';
                const csvFiles = list.filter(f =>
                  !f.attrs.isDirectory() &&
                  f.filename.toLowerCase().endsWith('.csv') &&
                  matchesRemoteImportPattern(f.filename, pat)
                );
                ...
                if (csvFiles.length > 1 && !selectedFileName) {
                  conn.end();
                  resolve({
                    success: true,
                    needsFileSelection: true,
                ...
                let targetCsvFile;
                if (selectedFileName) {
                  targetCsvFile = csvFiles.find(f => f.filename === selectedFileName);
                  if (!targetCsvFile) {
                    conn.end();
                  ...
                  targetCsvFile = csvFiles[0];
                }

                const filePath = `${testConfig.connection.remote_directory}/${targetCsvFile.filename}`;
                ...
                stream.on('end', () => {
                  ...
                  try {
                    ...
                    resolve({
                      ...
                      fileName: targetCsvFile.filename,
...
router.post('/test-connection-and-download', authenticateToken, async (req, res) => {
  ...
  if (origin) {
    res.header('Access-Control-Allow-Origin', origin);
    res.header('Access-Control-Allow-Credentials', 'true');
    console.log('🌐 Test-connection CORS headers set for origin:', origin);
  }
  ...
  try {
    ...
    console.log('🔧 Test config created:', {
      ...
      pattern: testConfig.connection.file_pattern
    });

    try {
      console.log('📦 Loading SSH2 client...');
      const { Client } = await import('ssh2');
    ...
    console.error('❌ Unexpected error in test-connection-and-download:', error);
    console.error('Error stack:', error.stack);
    
    // Check if response was already sent
    if (!res.headersSent) {
      // Ensure CORS headers are included in final error responses
      const origin = req.headers.origin;
      if (origin) {
        res.header('Access-Control-Allow-Origin', origin);
        res.header('Access-Control-Allow-Credentials', 'true');
      ...
      res.status(500).json({ 
        success: false,
        error: 'Failed to test connection and download file',
        details: error.message,
...
// GET /api/import/health - Health check endpoint
