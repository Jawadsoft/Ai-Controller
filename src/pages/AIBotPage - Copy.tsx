...
const AIBotPage: React.FC<AIBotPageProps> = ({ 
  ...
  const [isPreparingToListen, setIsPreparingToListen] = useState(false); // true between audio end and recognition.onstart
  ...
  const recordingVolumeThresholdRef = useRef(0.04); // Threshold to detect actual silence vs ambient noise (lowered for better sensitivity in noisy environments)
  ...
  useEffect(() => {
    ...
    if (hash) {
      ...
      if (extractedDealerId) {
        ...
        fetchDealerById(extractedDealerId).then(async (dealerData) => {
          if (dealerData) {
            // Try to resume a prior session first
            const resumed = await tryResumeConversation(dealerData.id);
            if (!resumed) {
        ...
        fetchDealerFromHash(actualHash, vehicleIdentifier || undefined).then(async (dealerData) => {
          if (dealerData) {
            const resumed = await tryResumeConversation(dealerData.id);
            if (!resumed) {
              const newSessionId = `aibot_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  const startRecording = () => {
    ...
    recognition.interimResults = true; // Interim results improve detection of quiet/accented speech
    ...
    recognition.onresult = async (event) => {
      // With interimResults=true, this fires for both interim and final results.
      // Only act on the final result to avoid duplicate processing.
        ...
        audioRef.current.addEventListener('ended', () => {
          ...
          setIsPlaying(false);
            ...
            if (webSpeechPausedForPlaybackRef.current) {
              ...
              setIsPreparingToListen(true); // Show "Getting ready…" until onstart fires
              setTimeout(() => {
              ...
              }, 150); // Reduced from 700ms — audio has already ended, brief settle is enough
            }
  ...
  const startWebSpeechTestOnly = (source: 'manual' | 'auto') => {
    ...
    try {
      ...
      recognition.onstart = () => {
        ...
        setIsPreparingToListen(false); // Mic is now actually open — clear the "getting ready" state
        setIsWebSpeechTestActive(true);
      };

      recognition.onresult = (event: any) => {
        // With interimResults=true, only process the final result to avoid acting on partials.
        const result = event.results[event.results.length - 1];
        if (!result?.isFinal) return;
        const transcript = (result[0]?.transcript as string | undefined)?.trim() ?? '';
      ...
      recognition.onend = () => {
        ...
        webSpeechTestRef.current = null;
        setIsWebSpeechTestActive(false);
        setIsPreparingToListen(false); // Ensure cleared if onstart never fired
      };

      webSpeechTestRef.current = recognition;
      // Minimal delay to let the mic hardware switch from output to input mode.
      window.setTimeout(() => {
        try {
          recognition.start();
        } catch (e) {
      ...
      }, 50); // Reduced from 200ms — combined with the outer 150ms this is sufficient
    } catch (error) {
    ...
    return (
      ...
      >
        ...
        <div className="flex justify-between items-start gap-2 mb-1.5">
          ...
          <span className="text-base font-bold text-[#FF6B2B] whitespace-nowrap">{vehicle.price}</span>
        </div>
        ...
        <div className="mt-2 pt-2 border-t border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-[#FF6B2B] text-white hover:bg-[#e85f24] transition-colors"
              onClick={(e) => {
                e.stopPropagation();
                onSelect(vehicle);
              }}
            ...
            <button
              ...
              className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-[#5D6D7E] text-white hover:bg-[#4d5a68] transition-colors"
              onClick={(e) => {
                e.stopPropagation();
                onTestDrive(vehicle);
              }}
            >
              <img src={testDriveIconUrl} alt="" className="w-5 h-5" draggable={false} />
            </button>
            ...
            >
              {loadingCarfax ? (
                ...
                <img src={carfaxIconUrl} alt="" className="w-5 h-5" draggable={false} />
              )}
            </button>
  ...
  return (
    <div className="h-[100dvh] bg-[#F3F6FB] p-2 sm:p-4 overflow-hidden">
      ...
      <div className="max-w-6xl xl:max-w-7xl mx-auto h-full">
        <Card className="w-full h-full flex flex-col border border-[#5D6D7E]/20 shadow-2xl shadow-slate-300/35 rounded-3xl overflow-hidden bg-white">

          <CardHeader className="p-0 space-y-0 border-0">
          ...
          <CardContent className="flex-1 min-h-0 flex flex-col p-0">
            ...
            {/* Messages Area */}
            <div
              className="flex-1 min-h-0 overflow-y-auto overscroll-contain touch-pan-y px-2 sm:px-4 pb-4 bg-[#F3F6FB]"
              ref={scrollAreaRef}
            >
              ...
              <div className="daive-overlay-enter mx-3 mb-2 mt-0 relative select-none">
                ...
                >
                  ...
                  <div className="relative shrink-0 flex items-center justify-center z-10" style={{ width: 44, height: 44 }}>
                    {/* Glow halo */}
                    <div
                      className={`absolute rounded-full ${isPlaying ? 'daive-orb-speaking' : (isWebSpeechTestActive && !isPreparingToListen) ? 'daive-orb-listening' : 'daive-orb-idle'}`}
                      style={{
                        inset: -8, opacity: 0.35,
                        background: isPlaying ? 'radial-gradient(circle, rgba(255,107,43,0.8) 0%, transparent 70%)' : (isWebSpeechTestActive && !isPreparingToListen) ? 'radial-gradient(circle, rgba(0,200,180,0.8) 0%, transparent 70%)' : 'radial-gradient(circle, rgba(90,140,255,0.8) 0%, transparent 70%)',
                      }}
                    />
                    {/* Sphere */}
                    <div
                      className={`rounded-full relative overflow-hidden shadow-lg ${isPlaying ? 'daive-orb-speaking' : (isWebSpeechTestActive && !isPreparingToListen) ? 'daive-orb-listening' : 'daive-orb-idle'}`}
                      style={{
                        width: 44, height: 44,
                        background: isPlaying
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      ...
                      {(isWebSpeechTestActive || isPlaying) ? (
                        ...
                        <p className="text-white/45 text-xs leading-tight">
                          {isProcessing ? 'Thinking…' : isPreparingToListen ? 'Getting ready…' : 'Ready — say something'}
                        </p>
                      )}
                    </div>
                  </div>
            ...
            <div className="relative p-3 sm:p-4 border-t border-[#CFD8DC] bg-white">
                ...
                <div className="daive-overlay-enter absolute inset-0 z-20 flex items-center justify-between gap-3 px-4 rounded-b-3xl"
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <span className="text-white/80 text-sm font-medium truncate">
                      {isPlaying ? 'Daive is speaking…' : isProcessing ? 'Daive is thinking…' : isPreparingToListen ? 'Getting mic ready…' : 'Listening — say something or cancel to type'}
                    </span>
                  </div>
                  <button
                    onClick={() => {
        ...
        {isFiltersModalOpen && (
          ...
          >
            <div
              className="absolute inset-x-0 bottom-0 flex flex-col h-[90vh] rounded-t-3xl bg-white shadow-2xl overflow-hidden"
