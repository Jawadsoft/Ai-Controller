...
const migrations = [
  ...
  `,
  // Add dealer-specific scenario flows table for conversation monitor
  `
    CREATE TABLE IF NOT EXISTS daive_scenario_flows (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      dealer_id UUID REFERENCES dealers(id) ON DELETE CASCADE,
      scenario_key TEXT NOT NULL,
      label TEXT,
      step INTEGER NOT NULL CHECK (step >= 1 AND step <= 16),
      message_text TEXT,
      response_text TEXT,
      created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
      updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
      UNIQUE(dealer_id, scenario_key, step)
    );
  `,
  // Indexes for scenario flows
  `
    CREATE INDEX IF NOT EXISTS idx_daive_scenario_flows_dealer_id ON daive_scenario_flows(dealer_id);
    CREATE INDEX IF NOT EXISTS idx_daive_scenario_flows_scenario_key ON daive_scenario_flows(scenario_key);
    CREATE INDEX IF NOT EXISTS idx_daive_scenario_flows_step ON daive_scenario_flows(step);
  `,
  // Trigger to update updated_at on scenario flows
  `
    DO $$
    BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_daive_scenario_flows_updated_at') THEN
        CREATE TRIGGER update_daive_scenario_flows_updated_at BEFORE UPDATE ON daive_scenario_flows
          FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
      END IF;
    END $$;
  `,
  ...
  `
    ...
    ON CONFLICT (dealer_id) DO NOTHING;
  `,
  // Track dealership user logins for dashboard analytics
  `
  ...
  `,
  // test_drive_appointments table — created here so it exists before the UNIQUE constraint below
  `
  ...
  `,
  `
    CREATE INDEX IF NOT EXISTS idx_tda_dealer  ON test_drive_appointments(dealer_id);
    CREATE INDEX IF NOT EXISTS idx_tda_session ON test_drive_appointments(session_id);
    CREATE INDEX IF NOT EXISTS idx_tda_conv    ON test_drive_appointments(conversation_id);
    CREATE INDEX IF NOT EXISTS idx_tda_status  ON test_drive_appointments(dealer_id, status);
  `,
  // Unique constraint on session_id — enables ON CONFLICT upsert for reschedules
  `
    DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'uq_tda_session' AND conrelid = 'test_drive_appointments'::regclass
      ) THEN
        ALTER TABLE test_drive_appointments ADD CONSTRAINT uq_tda_session UNIQUE (session_id);
      END IF;
    END $$;
  `
];
