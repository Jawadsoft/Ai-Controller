/**
 * Update Vehicle Table with Trim and Type Data - CORRECTED VERSION
 * Uses body_style column to properly identify vehicle types
 * This ensures the AI bot has all fields needed for effective communication
 */

import { pool } from './src/database/connection.js';

console.log('🚗 Updating Vehicle Table with Trim and Type Data (Corrected)...');
...
async function updateVehicleTable() {
  ...
  try {
    ...
    sampleResult.rows.forEach((vehicle, index) => {
      console.log(`   ${index + 1}. ${vehicle.make} ${vehicle.model} ${vehicle.trim || 'NO TRIM'} - Body: "${vehicle.body_style}" - Type: ${vehicle.vehicle_type || 'NO TYPE'}`);
    });
    ...
    // Update 1: Set vehicle_type based on body_style (CORRECTED MAPPING)
    console.log('   🔄 Updating vehicle_type from body_style...');
    const updateTypeQuery = `
      UPDATE vehicles 
      SET vehicle_type = CASE 
        -- SUVs
        WHEN body_style = '4D Sport Utility' THEN 'SUV'
        
        -- Sedans
        WHEN body_style = '4D Sedan' THEN 'sedan'
        
        -- Trucks
        WHEN body_style IN ('4D Crew Cab', '2D Standard Cab', '4D SuperCrew') THEN 'truck'
        
        -- Convertibles
        WHEN body_style = '2D Convertible' THEN 'convertible'
        
        -- Coupes
        WHEN body_style = '2D Coupe' THEN 'coupe'
        
        -- Hatchbacks
        WHEN body_style IN ('4D Hatchback', '5D Hatchback') THEN 'hatchback'
        
        -- Vans
        WHEN body_style = '4D Passenger Van' THEN 'van'
    ...
    console.log(`   ✅ Updated ${updateTypeResult.rowCount} vehicles with vehicle_type from body_style`);
    
    // Update 2: Set trim based on model and price if trim is missing
      ...
      SET trim = CASE 
        ...
        WHEN make ILIKE '%ford%' AND model ILIKE '%explorer%' THEN 
          CASE 
            ...
            ELSE 'Base'
          END
        WHEN make ILIKE '%ford%' AND model ILIKE '%f-150%' THEN 
          CASE 
            WHEN price >= 60000 THEN 'Platinum'
            WHEN price >= 50000 THEN 'Lariat'
            WHEN price >= 40000 THEN 'XLT'
            WHEN price >= 35000 THEN 'STX'
            ELSE 'Base'
          END
        ...
        WHEN make ILIKE '%chevrolet%' AND model ILIKE '%equinox%' THEN 
          CASE 
            ...
            ELSE 'Base'
          END
        WHEN make ILIKE '%chevrolet%' AND model ILIKE '%camaro%' THEN 
          CASE 
            WHEN price >= 40000 THEN 'SS'
            WHEN price >= 30000 THEN 'LT'
            WHEN price >= 20000 THEN 'Base'
            ELSE 'Base'
          END
        WHEN make ILIKE '%chevrolet%' AND model ILIKE '%corvette%' THEN 
          CASE 
            WHEN price >= 80000 THEN 'Z06'
            WHEN price >= 60000 THEN 'Stingray'
            ELSE 'Base'
          END
    ...
    sampleUpdatedResult.rows.forEach((vehicle, index) => {
      console.log(`   ${index + 1}. ${vehicle.make} ${vehicle.model} ${vehicle.trim} - Type: ${vehicle.vehicle_type} - Body: "${vehicle.body_style}" - $${vehicle.price}`);
    });
