...
async function forceCleanupFunctions() {
  ...
  try {
    console.log('🧹 Force cleaning up duplicate functions...');
    ...
    console.log(`\n📊 Found ${functions.rows.length} functions with this name:`);
    ...
    // Try to drop each function individually using different approaches
    console.log('\n🗑️ Attempting to drop functions individually...');
    
    // Try dropping with no arguments first
    try {
      await client.query(`DROP FUNCTION IF EXISTS import_vehicle_from_csv();`);
      console.log('✅ Dropped function with no arguments');
    } catch (error) {
      console.log('⚠️ Could not drop function with no arguments:', error.message);
    }
    
    // Try dropping with common parameter patterns
    const commonSignatures = [
      'UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, BOOLEAN, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, DECIMAL, DECIMAL, TEXT, DECIMAL, DECIMAL, DECIMAL, DECIMAL, DECIMAL, DECIMAL, TEXT, INTEGER, TEXT',
      'UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, BOOLEAN, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, DECIMAL, DECIMAL, TEXT, DECIMAL, DECIMAL, DECIMAL, DECIMAL, DECIMAL, DECIMAL, TEXT, INTEGER, TEXT',
      'UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, BOOLEAN, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, DECIMAL, DECIMAL, TEXT, DECIMAL, DECIMAL, DECIMAL, DECIMAL, DECIMAL, DECIMAL, TEXT, INTEGER, TEXT'
    ];
    
    for (const signature of commonSignatures) {
      try {
        await client.query(`DROP FUNCTION IF EXISTS import_vehicle_from_csv(${signature});`);
        console.log(`✅ Dropped function with signature: ${signature.substring(0, 50)}...`);
      } catch (error) {
        console.log(`⚠️ Could not drop function with signature ${signature.substring(0, 50)}...: ${error.message}`);
      }
    }
    ...
    if (remainingFunctions.rows[0].count === 0) {
      console.log('✅ All functions dropped successfully!');
      
      // Now create the correct function
      console.log('\n🔧 Creating correct import_vehicle_from_csv function...');
      ...
      console.log(`\n📊 Final function count: ${finalCheck.rows[0].count}`);
      
      if (finalCheck.rows[0].count === 1) {
        console.log('🎯 Perfect! Import function is ready to use!');
      }
      
    } else {
      console.log('❌ Some functions still remain - manual cleanup may be needed');
    }
...
forceCleanupFunctions().catch(console.error);
