...
export const ELEVENLABS_VOICES = {
  ...
  mark: {
    ...
    name: 'Mark-Natural Conversation',
    description: 'Male, natural, conversational, friendly'
  }
};
...
export function getElevenLabsVoiceId(voiceName) {
  ...
  return voice ? voice.id : ELEVENLABS_VOICES.mark.id; // Default to Mark voice
}

// Get voice info by name
export function getElevenLabsVoiceInfo(voiceName) {
  const voice = ELEVENLABS_VOICES[voiceName?.toLowerCase()];
  return voice || ELEVENLABS_VOICES.mark; // Default to Mark voice
}
