...
async function setupImportFunction() {
  const client = await pool.connect();
  
  try {
    console.log('🔧 Setting up import_vehicle_from_csv function...');
    
    // Create the function with 28 parameters (latest version)
    const functionSQL = `
    ...
    console.error('❌ Error setting up import function:', error);
    throw error;
  } finally {
    client.release();
    await pool.end();
...
setupImportFunction().catch(console.error);
